...
def load_dataset(path="../node_level",dataset_name="",
    ...
    # 对称化 + 去重    # adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)
    ...
    p = math.exp(eps_a) / (1 + math.exp(eps_a))  # probability of NOT flipping an edge
    q = (priv_deg / (priv_deg * (2 * p - 1) + (n - 1) * (1 - p))) # sampling probability of each edge
    q = torch.clamp(q, 0, 1)  # n by 1 vector


    pij = perturbed_adj * torch.bernoulli(q.matmul(torch.ones(1, n)))  # n by n matrix




    est_edge_index = pij.to_sparse().coalesce()
    edge_index = est_edge_index.indices()  # long
    edge_weight = est_edge_index.values().float()  # float
