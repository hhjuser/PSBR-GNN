...
def load_dataset(path="../node_level",dataset_name="",
    ...
    n_communities = labels_truth.max().item() + 1
    ...
    priv_adj = rr_adj(adj_tensor, eps_a)

    pij = link_posterior_Blink_0(prior_spectral, priv_adj,  eps_a)
    pij = pij.clamp(0.0, 1.0)

    W = (pij >= alpha_rate).float()

    edge_index_w, edge_weight_w = dense_to_sparse_with_weights(W)


    return edge_index_w, edge_weight_w, edge_index_w, edge_weight_w, features, labels_truth, idx_train, idx_val, idx_test





def link_posterior_Blink_0(pi,priv_adj,eps_a):
    ...
    def estimate_posterior_blink(prior):
        ...
        pr_y_edge = 0.5 * (x - 1) * (x - 2) * p * p + 0.5 * x * (x - 1) * (1 - p) * (1 - p) - 1 * x * (x - 2) * p * (
                    1 - p)
        pr_y_no_edge = 0.5 * (x - 1) * (x - 2) * (1 - p) * (1 - p) + 0.5 * x * (x - 1) * p * p - 1 * x * (x - 2) * p * (
                    1 - p)
...
def calculate_chung_lu_prior(adj_tensor: torch.Tensor,
                             ...
                             idx_val: torch.Tensor = None ) -> torch.Tensor:

    community_labels = None

    if method == 'feature_MLP':
        community_labels = build_community_labels_with_mlp(
            features=features,
            labels_truth=labels_truth,
            idx_train=idx_train,
            idx_val=idx_val,
    ...
    if community_labels is None:
        raise RuntimeError("community_labels was not generated. Please check method.")


    if community_labels.dim() == 1:
        community_labels = F.one_hot(
            community_labels.to(torch.long),
            num_classes=n_communities
        ).float()

    else:
        community_labels = community_labels.float()

    community_labels = community_labels.to(adj_tensor.device)




    if adj_tensor.is_sparse:
        prior_matrix = torch.sparse_coo_tensor(
            torch.empty((2, 0), dtype=torch.long, device=adj_tensor.device),
            torch.empty((0,), dtype=torch.float32, device=adj_tensor.device),
            size=adj_tensor.shape,
            device=adj_tensor.device
        )
    else:
        prior_matrix = torch.zeros_like(adj_tensor).float()



    if prior_method == 'dcsbm':
        ...
        C = community_labels.float().to(device)


        C = C / C.sum(dim=1, keepdim=True).clamp_min(eps)

        N, K = C.shape


        if adj_tensor.is_sparse:
            clean_degree_vectors = torch.sparse.mm(adj_tensor.float(), C)  # [N, K]
        else:
            clean_degree_vectors = torch.matmul(adj_tensor.float(), C)  # [N, K]


        laplace = torch.distributions.Laplace(
            loc=torch.tensor(0.0, device=device),
            scale=torch.tensor(1.0 / eps_d, device=device)
        )

        noise = laplace.sample(clean_degree_vectors.shape)
        noisy_degree_vectors = clean_degree_vectors + noise


        noisy_degree_vectors = torch.clamp(noisy_degree_vectors, min=1.0)  # [N, K]


        d_hat = noisy_degree_vectors.sum(dim=1)


        M_tilde = C.T @ noisy_degree_vectors  # [K, K]


        cluster_size = C.sum(dim=0).clamp_min(eps)  # [K]


        n_i = cluster_size.view(K, 1)  # [K, 1]
        n_j = cluster_size.view(1, K)  # [1, K]

        M_mle = (n_j / (n_i + n_j)) * M_tilde + \
                (n_i / (n_i + n_j)) * M_tilde.T



        R = C.T @ (C * d_hat.view(-1, 1))  # [K, K]

        R = R + eps * torch.eye(K, device=device, dtype=R.dtype)

        R_inv = torch.linalg.pinv(R)


        M_norm = R_inv @ M_mle @ R_inv  # [K, K]


        Phi = C @ M_norm @ C.T  # [N, N]

        theta_outer = torch.outer(d_hat, d_hat)  # [N, N]


        prior_matrix = Phi * theta_outer


        prior_matrix.fill_diagonal_(0.0)



    return prior_matrix,community_labels
...
def make_hash_rule_mask(N, device, seed=42, dtype=torch.float32):

    row, col = torch.triu_indices(N, N, offset=1, device=device)

    # 一个确定性的对称 hash 规则
    h = (row.to(torch.int64) * 1000003 +
         col.to(torch.int64) * 9176 +
         int(seed)) & 1

    owner_is_row = (h == 0)

    rule_mask = torch.zeros((N, N), device=device, dtype=dtype)

    # 若 row 拥有，则保留方向 (row, col)
    rule_mask[row[owner_is_row], col[owner_is_row]] = 1.0

    # 若 col 拥有，则保留方向 (col, row)
    rule_mask[col[~owner_is_row], row[~owner_is_row]] = 1.0
...
def build_community_labels_with_mlp(
    features,
    labels_truth,
    idx_train,
    idx_val,
    ...
    model.eval()
    with torch.no_grad():
        logits, embeddings = model(features)  # logits: [N, num_classes]

    # 3. soft classification matrix C
    C = F.softmax(logits, dim=1)  # [N, num_classes]
