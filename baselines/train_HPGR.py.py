...
from utils_HPGR import load_dataset
...
parser.add_argument('--lr', type=float,nargs='+',  default=0.001,    # hard
                    help='Initial learning rate.')
parser.add_argument('--weight_decay', type=float, nargs='+', default=5e-4,   #5e-4,1e-3,1e-4,1e-5,0,0.0001
                    help='Weight decay (L2 loss on parameters).')
parser.add_argument('--dropout', type=float,nargs='+',  default=0.4,   #0,  1e-1,1e-2, 1e-3,0.5
                    help='Dropout rate (1 - keep probability).')
parser.add_argument('--hidden', type=int, default=16,  # ✅
                    help='Number of hidden units.')
parser.add_argument('--epsilon', type=float, nargs='+', default=[6],  #1,2,3,4,5,6,7,8
                    help='epsilon.')
parser.add_argument('--delta', type=float, nargs='+', default=[0.1],  #0.1, 0.4, 0.7, 0.9
                    help='delta of epsilon .')
parser.add_argument('--communities', type=float, nargs='+', default=[0],  #1,100,200,300,500,700,900
                    help='number of communities .')  #1,100,500,1000,1500,2000,2500,3000
parser.add_argument('--alpha', type=float, nargs='+', default=[0.25],  #0.1,0.25,0.5,0.75
                    help='tau for pseudo label of test nodes, 0 means using hard label, 1 means soft label, and other values represent temperature parameters')
...
def run(epsilon, delta, communities,alpha,dataset,sigma_degree):
    edge_index_w, edge_weight_w, edge_index_a, edge_weight_a, features, labels_truth, idx_train, idx_val, idx_test = load_dataset(
    ...
    if args.cuda:
        ...
        # adj = adj.cuda()
        edge_index_a = edge_index_a.cuda()
        edge_weight_a = edge_weight_a.cuda()
        edge_index_w = edge_index_w.cuda()
        edge_weight_w = edge_weight_w.cuda()
    ...
    def train(epoch):
        ...
        model.train()
        optimizer.zero_grad()
        output = model(features, edge_index_w, edge_weight_w)
        # output = model(features, edge_index_a, edge_weight_a)
...
with open('output_results_HPGR.txt', 'a') as f:
    print("******************************************************************************", file=f)
