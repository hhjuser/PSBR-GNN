...
def load_dataset(path="../node_level",dataset_name="",
    ...
    priv_deg = deg + noise
    priv_deg = torch.clamp(priv_deg, min=2.0, max=float(n - 2))  # 合法范围 float(n - 2)
    ...
    perturbed_adj = rr_adj(adj_tensor, eps_a)
    W = link_posterior_Blink_0(perturbed_adj, priv_deg, eps_a)  # 原始blink


    if use_topk:


        kth_p = torch.topk(W.flatten(), k=W.sum().long().item()).values[-1].item()
        W = W * (W >= kth_p).float()
...
def link_posterior_Blink_0(priv_adj,priv_deg, eps_a):
    ...
    def estimate_posterior_blink_chunked(prior, priv_adj, eps_a, batch_size=1000):
        """
        PyTorch 版本的分块后验估计，修复 transpose 报错并支持 GPU/CPU
        """
        # 确保 priv_adj 是 Tensor
        if not torch.is_tensor(priv_adj):
            priv_adj = torch.tensor(priv_adj)

        # 获取设备 (CPU 或 CUDA)，确保后续计算在同一设备进行
        device = priv_adj.device
        n = priv_adj.shape[0]

        # 将标量转换为 Tensor 以支持后续运算
        if not torch.is_tensor(eps_a):
            eps_a = torch.tensor(eps_a, device=device)

        p = 1.0 / (1.0 + torch.exp(eps_a))

        # 1. 预计算常数表 (Lookup Table)
        # x 的取值是 0, 1, 2
        x_vals = torch.tensor([0., 1., 2.], device=device)

        # 预计算 x=0,1,2 时的 edge 和 no_edge 概率值
        # 注意：这里全部使用 torch 运算
        term_edge = (0.5 * (x_vals - 1) * (x_vals - 2) * p * p +
                     0.5 * x_vals * (x_vals - 1) * (1 - p) * (1 - p) -
                     1 * x_vals * (x_vals - 2) * p * (1 - p))

        term_no_edge = (0.5 * (x_vals - 1) * (x_vals - 2) * (1 - p) * (1 - p) +
                        0.5 * x_vals * (x_vals - 1) * p * p -
                        1 * x_vals * (x_vals - 2) * p * (1 - p))

        # 结果容器
        pij_full = torch.zeros((n, n), device=device)

        # 处理 prior，确保它是 Tensor
        if not torch.is_tensor(prior):
            prior = torch.tensor(prior, device=device)
        ...
        for i in range(0, n, batch_size):
            ...
            # 取出当前块 (Batch x N)
            block_adj = priv_adj[i:end, :]

            # 【修复点】：PyTorch 中转置必须指定维度，或者使用 .t() (仅限2D)
            # 这里我们需要 priv_adj 的列切片的转置
            # 对应数学逻辑：x[i, j] = A[i, j] + A[j, i]
            # block_adj 对应 A[i:end, :]
            # 下面这行取 A[:, i:end] 然后转置，变成 (Batch x N) 的形状
            block_adj_t = priv_adj[:, i:end].t()

            # 计算 x (Batch x N)
            x_block = block_adj + block_adj_t

            # 查表 (利用 LongTensor 作为索引)
            x_indices = x_block.long()

            # 直接通过索引取值，极快
            pr_edge_block = term_edge[x_indices]
            pr_no_edge_block = term_no_edge[x_indices]

            # 处理 Prior (切片或标量)
            if prior.ndim == 0:  # 标量
                prior_block = prior
            else:  # 矩阵
                prior_block = prior[i:end, :]

            # 贝叶斯公式计算
            numerator = pr_edge_block * prior_block
            denominator = numerator + pr_no_edge_block * (1 - prior_block) + 1e-8

            # 填入结果
            pij_full[i:end, :] = numerator / denominator

            # 显式清理显存 (如果是 GPU 跑大图很有必要)
            del block_adj, block_adj_t, x_block, x_indices
            # torch.cuda.empty_cache() # 只有在显存极其紧张时取消注释，否则会拖慢速度
    ...
    # pij = estimate_posterior_blink_chunked(prior, priv_adj, eps_a, batch_size=1000)
