...
def load_dataset(path="../node_level",dataset_name="",
    ...
    print(f"原始边：")
    for i, row in enumerate(adj_tensor[:5]):  # [:10]
        idx_nz = torch.nonzero(row, as_tuple=True)[0]
        print(f"原始 第 {i} 行非零索引:", idx_nz.tolist())
        with open('output_edges.txt', 'a') as f:
            print(f"原始 第 {i} 行非零索引:", idx_nz.tolist(), file=f)
