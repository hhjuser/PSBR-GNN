...
from torch_geometric.datasets import Coauthor,CitationFull, Planetoid, FacebookPagePage,LastFMAsia
...
def load_bound_error(path="../node_level",dataset_name="",
              eps_edge=7,
              delta_eps=0.1,
              n_communities=7,
              sigma = 0
    ...
    priv_deg = deg + noise
    priv_deg = torch.clamp(priv_deg, min=1.0, max=float(n - 2))  # 合法范围 float(n - 2)
    ...
    print("dataset_name:", dataset_name,"epsilon:",eps_e, "eps_d:", eps_d, "eps_a:", eps_a, "communities:", n_communities,"sigma:",sigma)

    print("-------------------------------")
    adj_tensor = (adj_tensor.float() > 0).float()
    adj_tensor.fill_diagonal_(0)
    print("-------------~PX-PX------------------")

    # P_clean = build_clean_P(adj_tensor, community_one_hot)
    P_clean = build_clean_P_sparse(
        adj_tensor,
        community_one_hot,
        chunk_size=256
    )
    # clean_PA_MAE_bound = compute_clean_PA_MAE_bound(adj_tensor,  n_communities)
    # clean_PA_MAE_error = compute_MAE_error(adj_tensor, P_clean)
    # print(f"clean_PA_MAE_bound: {clean_PA_MAE_bound:.6f}")
    # print(f"clean_PA_MAE_error: {clean_PA_MAE_error:.6f}")

    print()
    "PX-AX (特征归一化）"
    X = F.normalize(features, p=2, dim=1)
    row_l1 = torch.norm(X, p=1, dim=1)
    C_X = row_l1.max()
    print("C_X = max_j ||X_j||_1:", C_X.item())
    F_dim = X.shape[1]
    n = features.shape[0]
    ...
    E_count = torch.sum(d)
    K = n_communities
    # B_K = (4 * (sum_d2 - E_count) / K
    #         + 2 * (E_count ** 2) / (K ** 2))
    B_K = (2 * (E_count ** 2) / (K ** 2)) + (2 * E_count)/K
    bound = (C_X  / (n * F_dim)) * B_K
    print(f"clean_PXAX_MAE_bound:{bound.item():.6f}")
    PX_clean = P_clean @ X
    PX_error = adj_tensor @ X
    prop_error_l1 = torch.sum(torch.abs(PX_clean - PX_error)) / (n * F_dim)
    print(f"clean_PXAX_MAE_error:{prop_error_l1.item():.6f}")
    ...
    perturbed_adj = rr_adj(adj_tensor, eps_e)
    # naive_rr_MAE_bound = compute_naive_rr_mse_bound(adj_tensor, eps_e)
    # print(f"naive_rr_MAE_bound: {naive_rr_MAE_bound:.6f}")
    # naive_rr_MAE_error = compute_MAE_error(adj_tensor, perturbed_adj)
    # print(f"naive_rr_MAE_error: {naive_rr_MAE_error:.6f}")

    print()
    q = 1.0 / (1.0 + torch.exp(torch.tensor(float(eps_e))))
    naive_rr_l1_bound = n * (n - 1) * q
    naive_rr_MAE_bound = C_X * naive_rr_l1_bound / (n * F_dim)
    print(f"naive_rrAX_MAE_bound: {naive_rr_MAE_bound:.6f}")

    adj_tensor_clean = adj_tensor @ X
    perturbed_adj_error = perturbed_adj @ X
    naive_rr_MAE_error = torch.sum(torch.abs(adj_tensor_clean - perturbed_adj_error)) / (n * F_dim)
    print(f"naive_rrAX_MAE_error: {naive_rr_MAE_error:.6f}")
    ...
    "请设置sigma=1"
    # noise_P_A_MAE_bound = compute_reweighted_MAE_bound(
    #     adj_tensor,
    #     community_one_hot,
    #     eps_a,
    #     eps_d)
    # noise_P_A_MAE_bound = noise_P_A_MAE_bound + clean_PA_MAE_bound
    # print(f"noise_P_A_MAE_bound: {noise_P_A_MAE_bound:.6f}")
    # noise_P_A_MAE_error = compute_MAE_error(adj_tensor, prior_spectral)
    # print(f"noise_P_A_MAE_error: {noise_P_A_MAE_error:.6f}")

    print()
    n, F_dim = X.shape
    A = adj_tensor.float()
    d = A.sum(dim=1)
    E_count = d.sum()  # 如果 A 是无向对称邻接矩阵，这里是双向计数
    l1_bound = 2 * E_count + n / (2 * eps_d)
    noise_P_A_MAE_bound = C_X * l1_bound / (n * F_dim)
    print(f"noise_PXAX_MAE_bound: {noise_P_A_MAE_bound:.6f}")
    adj_tensor_clean = adj_tensor @ X
    perturbed_adj_error = prior_spectral @ X
    noise_P_A_MAE_error = torch.sum(torch.abs(adj_tensor_clean - perturbed_adj_error)) / (n * F_dim)
    print(f"noise_PXAX_MAE_error: {noise_P_A_MAE_error:.6f}")


    print("-------------~P0X-AX------------------")

    "请设置sigma=0"
    # MAE_P0_A_bound = compute_MAE_P0_A_bound(
    #     adj_tensor,
    #     community_one_hot,
    #     eps_a
    # )
    # MAE_P0_A_bound = MAE_P0_A_bound.item() + clean_PA_MAE_bound
    # print(f"MAE_P0_A_bound: {MAE_P0_A_bound:.6f}")
    # MAE_P0_A_error = compute_MAE_error(adj_tensor, prior_spectral)
    # print(f"MAE_P0_A_error: {MAE_P0_A_error:.6f}")
    ...
    E_count = d.sum()
    K = n_communities
    f = 1.0 / (1.0 + math.exp(float(eps_e)))
    l1_bound = 2 * (1 - f) * E_count + n * K * f
    noise_P0_A_MAE_bound = (C_X * l1_bound) / (n * F_dim)
    print(f"noise_P0XAX_MAE_bound: {noise_P0_A_MAE_bound:.6f}")

    Q = community_one_hot.float()
    K = Q.shape[1]
    group_sizes = Q.sum(dim=0)  # [K], each entry is |G_a|
    g_max  = group_sizes.max()  # scalar tensor
    print(f"g_max : {g_max :.6f}")
    q_g = 1.0 / (1.0 + torch.exp(torch.tensor(float(eps_e), device=A.device)))
    term_rr_direct = n * K * q_g

    term_decoder = 2.0 * g_max * E_count * q_g

    term_clean_recon = 2.0 * (E_count ** 2) / (K ** 2) + (2 * E_count)/K

    bracket = term_rr_direct + term_decoder + term_clean_recon

    # final MAE bound
    noise_P0XAX_MAE_bound_add = (C_X / (n * F_dim)) * bracket
    print(f"noise_P0XAX_MAE_bound_add: {noise_P0XAX_MAE_bound_add:.6f}")

    adj_tensor_clean = adj_tensor @ X
    perturbed_adj_error = prior_spectral @ X
    noise_P0_A_MAE_error = torch.sum(torch.abs(adj_tensor_clean - perturbed_adj_error)) / (n * F_dim)
    print(f"noise_P0XAX_MAE_error: {noise_P0_A_MAE_error:.6f}")

    print("-------------------------------")

    with open('output_24_bound_error.txt', 'a') as f:
        print("dataset_name:", dataset_name, "epsilon:", eps_e, "eps_d:", eps_d, "eps_a:", eps_a, "communities:",
              comm,"=",n_communities, "sigma:", sigma,file=f)
        print("-------------~PX-PX------------------",file=f)
        print("C_X = max_j ||X_j||_1:", C_X.item(),file=f)
        print(f"clean_PXAX_MAE_bound:{bound.item():.6f}",file=f)
        print(f"clean_PXAX_MAE_error:{prop_error_l1.item():.6f}",file=f)
        print("--------------~AX-AX-----------------",file=f)
        print(f"naive_rrAX_MAE_bound: {naive_rr_MAE_bound:.6f}",file=f)
        print(f"naive_rrAX_MAE_error: {naive_rr_MAE_error:.6f}",file=f)
        print("-------------~PX-AX------------------",file=f)
        print(f"noise_PXAX_MAE_bound: {noise_P_A_MAE_bound:.6f}",file=f)
        print(f"noise_PXAX_MAE_error: {noise_P_A_MAE_error:.6f}",file=f)
        print("-------------~P0X-AX------------------",file=f)
        print(f"g_max: {g_max:.6f}",file=f)
        print(f"noise_P0XAX_MAE_bound: {noise_P0_A_MAE_bound:.6f}",file=f)
        print(f"noise_P0XAX_MAE_bound_add: {noise_P0XAX_MAE_bound_add:.6f}",file=f)
        print(f"noise_P0XAX_MAE_error: {noise_P0_A_MAE_error:.6f}",file=f)
        print("-------------------------------",file=f)
